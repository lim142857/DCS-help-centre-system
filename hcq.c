...
char *print_currently_serving(Ta *ta_list) {
	...
	// if no TAs are currently working
    if (ta_list == NULL) {
		int size = asprintf(&full_str,"No TAs are currently working.\r\n");
		if(size == -1){
			perror("[hcq] print_currently_serving asprintf\n");
        	exit(1);
		}
        return full_str;
    }

	Ta *ta_list_tail = ta_list;//store the list tail for traversing list second time

	//	loop through the list to add up all of the string lengths
	int total_length = 0;
	int each_length = 0;
	char *temp_str;
	while (ta_list != NULL) {
       	if (ta_list->current_student != NULL) {
		   	each_length = asprintf(&temp_str,"TA: %s is serving %s.\r\n",ta_list->name,ta_list->current_student->name);
       	} else {
			each_length = asprintf(&temp_str,"TA: %s has no student\r\n", ta_list->name);
      	}
		//error handling
		if(each_length == -1){
			perror("[hcq] print_currently_serving asprintf\n");
			...
			total_length += each_length;//return value does not include null terminator
			free(temp_str);
		}
       	ta_list = ta_list->next;
    }

	//  dynamically allocate space for full string
	full_str = malloc(sizeof(char)*(total_length+1));//add one for the null terminator
	full_str[0]='\0';//ensure full_str contains a valid string
	
	//	loop again to copy those strings into full_str
	ta_list = ta_list_tail;
	int size = 0;
	while (ta_list != NULL) {
       	if (ta_list->current_student != NULL) {
		   	size = asprintf(&temp_str,"TA: %s is serving %s.\r\n",ta_list->name,ta_list->current_student->name);
       	} else {
			size = asprintf(&temp_str,"TA: %s has no student\r\n", ta_list->name);
      	}
		//error handling
		if(size == -1){
			perror("[hcq] print_currently_serving asprintf\n");
			...
			strcat(full_str,temp_str);//return value does not include null terminator
			free(temp_str);
		}
       	ta_list = ta_list->next;
    }
...
char *print_full_queue(Student *stu_list) {    
	Student *stu_list_head = stu_list;//store the list head for traversing list second time
	...
	char *temp_str;
    while (stu_list != NULL) {
		each_length = asprintf(&temp_str,"Student %s:%s\r\n",stu_list->name, stu_list->course->code);
		if(each_length == -1){
			perror("[hcq] print_full_queue asprintf\n");
        	exit(1);
		}else{
			total_length += each_length;//return value does not include null terminator
			free(temp_str);
	...
	//  dynamically allocate space for full string
	char *full_str = malloc(sizeof(char)*(total_length+strlen("Full Queue\r\n")+1));//add one for the null terminator
	full_str[0]='\0';//ensure full_str contains a valid string
	strcat(full_str,"Full Queue\r\n");
	
	//	loop again to copy those strings into full_str
	stu_list = stu_list_head;
	while (stu_list != NULL) {
		int size = asprintf(&temp_str,"Student %s:%s\r\n",stu_list->name, stu_list->course->code);
		if(size == -1){
			perror("[hcq] print_full_queue asprintf\n");
        	exit(1);
		}else{
